...
export class EmailResolverBase {
  ...
  async createEmail(@graphql.Args() args: CreateEmailArgs): Promise<Email> {
    return await this.service.createEmail({
      ...
      data: {
        ...
        mailbox: args.data.mailbox
          ? {
              connect: args.data.mailbox,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => Mailbox, {
    nullable: true,
    name: "mailbox",
  })
  async getMailbox(@graphql.Parent() parent: Email): Promise<Mailbox | null> {
    const result = await this.service.getMailbox(parent.id);
